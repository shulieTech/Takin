...
public abstract class ToStringStyle implements Serializable {
    ...
    /**
     * The default toString style. Using the <code>Person</code> example from {@link
     * ToStringBuilder}, the output would look like this: <p>
     * <pre>
    ...
    /**
     * The multi line toString style. Using the <code>Person</code> example from {@link
     * ToStringBuilder}, the output would look like this: <p>
     * <pre>
     * Person@182f0db[
     *   name=John Doe
    ...
    /**
     * The no field names toString style. Using the <code>Person</code> example from {@link
     * ToStringBuilder}, the output would look like this: <p>
     * <pre>
    ...
    /**
     * The simple toString style. Using the <code>Person</code> example from {@link
     * ToStringBuilder}, the output would look like this: <p>
     * <pre>
     * John Doe,33,false
     * </pre>
    ...
    /**
     * The no class name toString style. Using the <code>Person</code> example from {@link
     * ToStringBuilder}, the output would look like this: <p>
     * <pre>
    ...
    /**
     * The JSON toString style. Using the <code>Person</code> example from {@link ToStringBuilder},
     * the output would look like this: <p>
     * <pre>
     * {"name": "John Doe", "age": 33, "smoker": true}
     * </pre>
     * <p> <strong>Note:</strong> Since field names are mandatory in JSON, this ToStringStyle will
     * throw an {@link UnsupportedOperationException} if no field name is passed in while appending.
     * Furthermore This ToStringStyle will only generate valid JSON if referenced objects also
     * produce JSON when calling {@code toString()} on them.
     *
    ...
    /**
     * <p> A registry of objects used by <code>reflectionToString</code> methods to detect cyclical
     * object references and avoid infinite loops. </p>
     */
    ...
    /**
     * <p> Returns the registry of objects being traversed by the <code>reflectionToString</code>
     * methods in the current thread. </p>
     *
    ...
    /**
     * <p> Returns <code>true</code> if the registry contains the given object. Used by the
     * reflection methods to avoid infinite loops. </p>
     *
    ...
    /**
     * <p> Unregisters the given object. </p> <p> <p> Used by the reflection methods to avoid
     * infinite loops. </p>
     *
    ...
    /**
     * <p>Append to the <code>toString</code> the superclass toString.</p> <p>NOTE: It assumes that
     * the toString has been created from the same ToStringStyle. </p> <p> <p>A <code>null</code>
     * <code>superToString</code> is ignored.</p>
     *
    ...
    /**
     * <p>Append to the <code>toString</code> another toString.</p> <p>NOTE: It assumes that the
     * toString has been created from the same ToStringStyle. </p> <p> <p>A <code>null</code>
     * <code>toString</code> is ignored.</p>
     *
     * @param buffer   the <code>StringBuffer</code> to populate
    ...
    /**
     * <p>Append to the <code>toString</code> an <code>Object</code> value, printing the full
     * <code>toString</code> of the <code>Object</code> passed in.</p>
     *
     * @param buffer     the <code>StringBuffer</code> to populate
    ...
    /**
     * <p>Append to the <code>toString</code> an <code>Object</code>, correctly interpreting its
     * type.</p> <p> <p>This method performs the main lookup by Class type to correctly route
     * arrays, <code>Collections</code>, <code>Maps</code> and <code>Objects</code> to the
     * appropriate method.</p> <p> <p>Either detail or summary views can be specified.</p> <p> <p>If
     * a cycle is detected, an object will be appended with the <code>Object.toString()</code>
     * format.</p>
     *
     * @param buffer    the <code>StringBuffer</code> to populate
    ...
    protected void appendInternal(final StringBuffer buffer, final String fieldName, final Object value, final boolean detail) {
        ...
        try {
            ...
            } else if (value instanceof ByteBuffer || value instanceof InputStream || value instanceof Reader
                    || value instanceof Channel || value instanceof Future) {
    ...
    /**
     * <p>Append to the <code>toString</code> an <code>Object</code> value that has been detected to
    ...
    /**
     * <p>Append to the <code>toString</code> an <code>Object</code> value, printing the full detail
     * of the <code>Object</code>.</p>
     *
     * @param buffer    the <code>StringBuffer</code> to populate
     * @param fieldName the field name, typically not used as already appended
     * @param value     the value to add to the <code>toString</code>, not <code>null</code>
    ...
    protected void appendDetail(final StringBuffer buffer, final String fieldName, final Object value) {
        ...
        if (null != value) {
            if (ClassUtils.isPrimitiveOrWrapper(value.getClass())) {
                ...
                if (limitSize != -1 && totalSize + ((String) value).length() > limitSize && limitSize - totalSize > 0) {
                    buffer.append((String) value, 0, (int) (limitSize - totalSize));
                } else {
                    buffer.append(value);
                }
            } else {
        ...
        if (null != map && !map.isEmpty()) {
            ...
            for (Map.Entry<?, ?> item : map.entrySet()) {
                ...
                if (item == null) {
                    ...
                    appendInternal(buffer, "k", item.getKey(), arrayContentDetail);
                    buffer.append(fieldNameValueSeparator);
                    appendInternal(buffer, "v", item.getValue(), arrayContentDetail);
                }
                i++;
            }
        }
    ...
    /**
     * <p>Append to the <code>toString</code> an <code>Object</code> value, printing a summary of
    ...
    /**
     * <p>Append to the <code>toString</code> an indicator for <code>null</code>.</p> <p> <p>The
     * default indicator is <code>'&lt;null&gt;'</code>.</p>
     *
     * @param buffer    the <code>StringBuffer</code> to populate
     * @param fieldName the field name, typically not used as already appended
     */
    ...
    /**
     * <p>Append to the <code>toString</code> a size summary.</p> <p> <p>The size summary is used to
     * summarize the contents of <code>Collections</code>, <code>Maps</code> and arrays.</p> <p>
     * <p>The output consists of a prefix, the passed in size and a suffix.</p> <p> <p>The default
     * format is <code>'&lt;size=n&gt;'</code>.</p>
     *
     * @param buffer    the <code>StringBuffer</code> to populate
     * @param fieldName the field name, typically not used as already appended
    ...
    /**
     * <p>Is this field to be output in full detail.</p> <p> <p>This method converts a detail
     * request into a detail level. The calling code may request full detail (<code>true</code>),
     * but a subclass might ignore that and always return <code>false</code>. The calling code may
     * pass in <code>null</code> indicating that it doesn't care about the detail level. In this
     * case the default detail level is used.</p>
     *
    ...
    /**
     * <p>Gets the short class name for a class.</p> <p> <p>The short class name is the classname
     * excluding the package name.</p>
     *
    ...
    /**
     * <p>Sets the array start text.</p> <p> <p><code>null</code> is accepted, but will be converted
    ...
    /**
     * <p>Sets the array end text.</p> <p> <p><code>null</code> is accepted, but will be converted
     * to an empty String.</p>
     *
    ...
    /**
     * <p>Sets the array separator text.</p> <p> <p><code>null</code> is accepted, but will be
    ...
    /**
     * <p>Sets the content start text.</p> <p> <p><code>null</code> is accepted, but will be
     * converted to an empty String.</p>
     *
    ...
    /**
     * <p>Sets the content end text.</p> <p> <p><code>null</code> is accepted, but will be converted
     * to an empty String.</p>
     *
    ...
    /**
     * <p>Sets the field name value separator text.</p> <p> <p><code>null</code> is accepted, but
     * will be converted to an empty String.</p>
     *
    ...
    /**
     * <p>Sets the field separator text.</p> <p> <p><code>null</code> is accepted, but will be
     * converted to an empty String.</p>
     *
    ...
    /**
     * <p>Sets the text to output when <code>null</code> found.</p> <p> <p><code>null</code> is
     * accepted, but will be converted to an empty String.</p>
     *
    ...
    /**
     * <p>Gets the start text to output when a <code>Collection</code>, <code>Map</code> or array
     * size is output.</p> <p> <p>This is output before the size value.</p>
     *
    ...
    /**
     * <p>Sets the start text to output when a <code>Collection</code>, <code>Map</code> or array
     * size is output.</p> <p> <p>This is output before the size value.</p> <p> <p><code>null</code>
     * is accepted, but will be converted to an empty String.</p>
     *
    ...
    /**
     * <p>Gets the end text to output when a <code>Collection</code>, <code>Map</code> or array size
     * is output.</p> <p> <p>This is output after the size value.</p>
     *
    ...
    /**
     * <p>Sets the end text to output when a <code>Collection</code>, <code>Map</code> or array size
     * is output.</p> <p> <p>This is output after the size value.</p> <p> <p><code>null</code> is
     * accepted, but will be converted to an empty String.</p>
     *
    ...
    /**
     * <p>Gets the start text to output when an <code>Object</code> is output in summary mode.</p>
     * <p> <p>This is output before the size value.</p>
     *
    ...
    /**
     * <p>Sets the start text to output when an <code>Object</code> is output in summary mode.</p>
     * <p> <p>This is output before the size value.</p> <p> <p><code>null</code> is accepted, but
     * will be converted to an empty String.</p>
     *
    ...
    /**
     * <p>Gets the end text to output when an <code>Object</code> is output in summary mode.</p> <p>
    ...
    /**
     * <p>Sets the end text to output when an <code>Object</code> is output in summary mode.</p> <p>
     * <p>This is output after the size value.</p> <p> <p><code>null</code> is accepted, but will be
     * converted to an empty String.</p>
     *
    ...
    /**
     * <p>Default <code>ToStringStyle</code>.</p> <p> <p>This is an inner class rather than using
    ...
    private static final class DefaultToStringStyle extends ToStringStyle {
        ...
        /**
         * <p>Constructor.</p> <p> <p>Use the static constant rather than instantiating.</p>
         */
    ...
    /**
     * <p><code>ToStringStyle</code> that does not print out the field names.</p> <p> <p>This is an
     * inner class rather than using <code>StandardToStringStyle</code> to ensure its immutability.
     */
    ...
    /**
     * <p><code>ToStringStyle</code> that prints out the short class name and no identity
     * hashcode.</p> <p> <p>This is an inner class rather than using <code>StandardToStringStyle</code>
    ...
    /**
     * <p><code>ToStringStyle</code> that does not print out the classname, identity hashcode,
     * content start or field name.</p> <p> <p>This is an inner class rather than using
     * <code>StandardToStringStyle</code> to ensure its immutability.</p>
     */
    ...
    /**
     * <p><code>ToStringStyle</code> that outputs on multiple lines.</p> <p> <p>This is an inner
     * class rather than using <code>StandardToStringStyle</code> to ensure its immutability.</p>
     */
    ...
    /**
     * <p><code>ToStringStyle</code> that does not print out the classname and identity hashcode but
     * prints content start and field names.</p> <p> <p>This is an inner class rather than using
     * <code>StandardToStringStyle</code> to ensure its immutability.</p>
     */
    ...
    /**
     * <p> <code>ToStringStyle</code> that outputs with JSON format. </p> <p> <p> This is an inner
     * class rather than using <code>StandardToStringStyle</code> to ensure its immutability. </p>
     *
     * @see <a href="http://json.org">json.org</a>
     * @since 3.4
    ...
    public ToStringStyle setLimitSize(long limitSize) {
        this.limitSize = limitSize;
        return this;
    }

    public ToStringStyle setTotalSize(long totalSize) {
