...
public class RedisClusterClientConstructorInterceptor extends AroundInterceptor {
    ...
    public void doBefore(Advice advice) {
        ...
        try {
            ...
            final Iterable<RedisURI> iterable = (Iterable<RedisURI>) args[1];
            List<RedisURI> list = new ArrayList<RedisURI>();
            if (iterable != null) {
                Iterator<RedisURI> it = iterable.iterator();
                while (it.hasNext()) {
                    list.add(it.next());
                }
            }
            manager.setDynamicField(target, LettuceConstants.DYNAMIC_FIELD_REDIS_URIS, list);
        } catch (Throwable t) {
