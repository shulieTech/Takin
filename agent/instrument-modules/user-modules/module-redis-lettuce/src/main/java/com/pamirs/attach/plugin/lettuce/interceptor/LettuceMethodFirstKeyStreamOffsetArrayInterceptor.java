...
/**
 * 第一个参数或者第二参数或者是第三参数是XReadArgs.StreamOffset数组类型
 */
public class LettuceMethodFirstKeyStreamOffsetArrayInterceptor extends ParametersWrapperInterceptorAdaptor {
    ...
    protected Object[] getParameter0(Advice advice) {
        ...
        if (args[0] instanceof XReadArgs.StreamOffset[]) {
            XReadArgs.StreamOffset[] orgArray = (XReadArgs.StreamOffset[]) args[0];
            if (orgArray != null && orgArray.length != 0) {
                XReadArgs.StreamOffset[] newArray = new XReadArgs.StreamOffset[orgArray.length];
                for (int i = 0, len = orgArray.length; i < len; i++) {
                    XReadArgs.StreamOffset orgOffset = orgArray[i];
                    XReadArgs.StreamOffset streamOffset = XReadArgs.StreamOffset.from(toClusterTestKey(orgOffset.getName()), orgOffset.getOffset());
                    newArray[i] = streamOffset;
                }
                args[0] = newArray;
            }
        ...
        if (args.length > 1 && args[1] instanceof XReadArgs.StreamOffset[]) {
            XReadArgs.StreamOffset[] orgArray = (XReadArgs.StreamOffset[]) args[1];
            if (orgArray != null && orgArray.length != 0) {
                XReadArgs.StreamOffset[] newArray = new XReadArgs.StreamOffset[orgArray.length];
                for (int i = 0, len = orgArray.length; i < len; i++) {
                    XReadArgs.StreamOffset orgOffset = orgArray[i];
        ...
        if (args.length > 2 && args[2] instanceof XReadArgs.StreamOffset[]) {
            XReadArgs.StreamOffset[] orgArray = (XReadArgs.StreamOffset[]) args[2];
            if (orgArray != null && orgArray.length != 0) {
                XReadArgs.StreamOffset[] newArray = new XReadArgs.StreamOffset[orgArray.length];
                for (int i = 0, len = orgArray.length; i < len; i++) {
                    XReadArgs.StreamOffset orgOffset = orgArray[i];
