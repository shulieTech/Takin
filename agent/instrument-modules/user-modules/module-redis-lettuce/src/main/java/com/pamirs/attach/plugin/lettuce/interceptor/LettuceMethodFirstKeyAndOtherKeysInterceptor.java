 ...
 */
public class LettuceMethodFirstKeyAndOtherKeysInterceptor extends ParametersWrapperInterceptorAdaptor {
    ...
    protected Object[] getParameter0(Advice advice) {
        ...
        args[0] = toClusterTestKey(args[0]);

        if (args.length > 1 && args[1] instanceof Object[]) {
            Object[] orgArray = (Object[]) args[1];
            if (orgArray != null && orgArray.length != 0) {
                Object[] newArray = new Object[orgArray.length];
                for (int i = 0, len = orgArray.length; i < len; i++) {
                    newArray[i] = toClusterTestKey(orgArray[i]);
                }
                args[1] = newArray;
            }
        }

        if (args.length > 2 && args[2] instanceof Object[]) {
            Object[] orgArray = (Object[]) args[2];
            if (orgArray != null && orgArray.length != 0) {
                Object[] newArray = new Object[orgArray.length];
                for (int i = 0, len = orgArray.length; i < len; i++) {
                    newArray[i] = toClusterTestKey(orgArray[i]);
