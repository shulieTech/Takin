        ...
        if (Pradar.isClusterTest()) {
            try {
                ...
                if (useTable) {
                    ...
                    return new PressureConnection(dataSourcePerformanceTest, dataSourcePerformanceTest.getConnection(), dataSourcePerformanceTest.getUrl(), dataSourcePerformanceTest.getUsername(), dbConnectionKey, dbType);
                }
            } catch (Throwable e) {
        ...
        if (Pradar.isClusterTest()) {
            try {
                ...
                if (useTable) {
                    ...
                    return new PressureConnection(dataSourcePerformanceTest, dataSourcePerformanceTest.getConnection(username, password), dataSourcePerformanceTest.getUrl(), dataSourcePerformanceTest.getUsername(), dbConnectionKey, dbType);
                }
            } catch (Throwable e) {
                ErrorReporter.Error error = ErrorReporter.buildError()
                        .setErrorType(ErrorTypeEnum.DataSource)
    ...
    public void close() {
        if (dataSourcePerformanceTest != null) {
            try {
                ...
                logger.error("[dbcp] pressurement datasource close err!", e);
            }
        }
    }
}
