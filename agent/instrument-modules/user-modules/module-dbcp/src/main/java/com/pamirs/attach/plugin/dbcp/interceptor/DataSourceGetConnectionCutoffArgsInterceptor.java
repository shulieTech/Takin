...
public class DataSourceGetConnectionCutoffArgsInterceptor extends CutoffInterceptorAdaptor {
    ...
    public DataSourceGetConnectionCutoffArgsInterceptor() {
        addListener();
    }
    ...
    private void addListener() {
        ...
        EventRouter.router().addListener(new PradarEventListener() {
            ...
            public EventResult onEvent(IEvent event) {
                ...
                return EventResult.success("dbcp-plugin");
            }
            ...
            public EventResult onEvent(IEvent event) {
                ...
                for (ShadowDatabaseConfig config : target) {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (StringUtils.equalsIgnoreCase(DbUrlUtils.getKey(config.getUrl(), config.getUsername()),
                            ...
                            try {
                                value.close();
                                logger.info("module-dbcp: destroyed shadow table datasource success. url:{} ,username:{}", entry.getKey().getUrl(), entry.getKey().getUsername());
                            } catch (Throwable e) {
                                logger.error("module-dbcp: closed datasource err! target:{}, url:{} username:{}", entry.getKey().getDataSource().hashCode(), entry.getKey().getUrl(), entry.getKey().getUsername(), e);
                            }
                            break;
                        }
                    }
                ...
                return EventResult.success("module-dbcp: destroyed shadow table datasource success.");
            }
