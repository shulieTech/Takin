 ...
 */
public class CacheKeyInterceptor0 extends AroundInterceptor {
    @Override
    public void doBefore(Advice advice) {
        ...
        if (!(element.getObjectKey() instanceof ClusterTestCacheWrapperKey)) {
            ...
            advice.changeParameter(0, ele);
        }
        if (args.length > 1 && args[1] instanceof Element) {
            Element element1 = (Element) args[1];
            Element ele = new Element(new ClusterTestCacheWrapperKey(element1.getObjectKey())
                    , element1.getObjectValue()
                    , element1.getVersion()
                    , element1.getCreationTime()
                    , element1.getLastAccessTime()
                    , element1.getHitCount()
                    , element1.usesCacheDefaultLifespan()
                    , element1.getTimeToLive()
                    , element1.getTimeToIdle()
                    , element1.getLastUpdateTime());
            advice.changeParameter(1, ele);
        }
    }
