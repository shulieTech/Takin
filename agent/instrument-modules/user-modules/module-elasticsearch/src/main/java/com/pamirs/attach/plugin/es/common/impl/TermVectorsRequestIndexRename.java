 ...
 */
public class TermVectorsRequestIndexRename extends AbstractReadRequestIndexRename {
    @Override
    ...
    public List<String> reindex0(Object target) {
        TermVectorsRequest req = (TermVectorsRequest) target;
        ...
        if (!Pradar.isClusterTestPrefix(index)) {
            index = Pradar.addClusterTestPrefix(req.index());
        }
        req.index(index);
        return Arrays.asList(index);
    }
