 ...
 */
public class ConsumerCommitSyncInterceptor extends AbstractProxiedConsumerInterceptor {

    @Override
    protected Object doCutoff(ConsumerProxy consumerProxy, Advice advice) {
        Object[] args = advice.getParameterArray();
        if (args.length == 1 && args[0] instanceof Map) {
            consumerProxy.commitSync((Map<TopicPartition, OffsetAndMetadata>)args[0]);
            return null;
        }
        if (args.length == 1 && args[0] instanceof Duration) {
            consumerProxy.commitSync();
            return null;
        }
        if (args.length == 2 && args[0] instanceof Map && args[1] instanceof Duration) {
            consumerProxy.commitSync((Map<TopicPartition, OffsetAndMetadata>)args[0]);
            return null;
        }
