...
import getBlacklistFormData from './BlacklistFormData';
...
const AddAndEditBlacklistDrawer: React.FC<Props> = props => {
  ...
  const queryBlacklistDetail = async () => {
    const {
      data: { success, data }
    } = await BlacklistService.queryBlacklistDetail({ blistId: id });
    if (success) {
      setState({
    ...
    return await new Promise(resolve => {
      state.form.validateFields(async (err, values) => {
        ...
        if (action === 'add') {
          const {
            data: { success, data }
          } = await BlacklistService.addBlacklist(result);
          if (success) {
  ...
  return (
    ...
    >
      <CommonForm
        ...
        formData={getBlacklistFormData(state, action, setState)}
        btnProps={{
          isResetBtn: false,
          isSubmitBtn: false
        }}
