...
package com.pamirs.tro.common.zk;
...
public class FormatUtils {
    ...
    /*public static String toEscapedJsArrayString(Collection<String> list) {
        if (list.isEmpty()) {
            return "[]";
        }

        StringWriter appender = new StringWriter(1024 * 4);
        appender.append('[');
        Iterator<String> it = list.iterator();

        appender.append('\"');
        try {
            StringEscapeUtils.escapeJavaScript(appender, it.next());
        } catch (IOException e) {
            e.printStackTrace();
        }
        appender.append('\"');
