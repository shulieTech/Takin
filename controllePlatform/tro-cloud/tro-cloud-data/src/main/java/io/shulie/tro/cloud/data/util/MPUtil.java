...
package io.shulie.tro.cloud.data.util;
...
import io.shulie.tro.cloud.data.dto.PageDTO;
...
public interface MPUtil<T> {
    ...
    default QueryWrapper<T> getTenantQW() {
        return this.getQW().eq("custom_id", CustomUtil.getUserId())
            .in(StringUtils.isNotBlank(RestContext.getFilterSql()), "user_id", RestContext.getFilterSql());
    }
